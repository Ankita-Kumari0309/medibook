...
// PUT /api/appointments/:id/complete     (no body)
// GET /api/doctor/appointments           (only if this route exists in your doctor routes)
...
function Avatar({ name }) {
  ...
  return (
    <div
      className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${colors[idx]}`}
    >
      {getInitials(name)}
    </div>
  );
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ring-1 ${b.bg} ${b.text} ${b.ring}`}
    >
      <span
            ...
            {actions.map((status) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-xs font-semibold transition-colors ${action.color}`}
                >
                  {action.label}
                </button>
              );
...
export default function DoctorAppointments() {
  ...
  // FETCH APPOINTMENTS
  //
  // Keep this endpoint as your existing
  // doctor-specific GET route.
  // ───────────────────────────────────
        ...
        <div className="divide-y divide-gray-50 max-h-[520px] overflow-y-auto">
          {loading ? (
            Array.from({
              ...
              <div
                key={i}
                className="grid grid-cols-[2fr_1.5fr_1.2fr_1.5fr_80px] gap-4 px-5 py-4 animate-pulse"
              >
